...
public abstract class CacheSupport implements CachePro {
	...
	public void verify() throws CacheException {
		try {
			...
			CacheException ce = new CacheException(e.getClass().getName() + ":" + e.getMessage());
			ce.initCause(e);
			throw ce;
		}
	}
	...
	// there was the wrong generic type defined in the older interface, because of
	// that we do not define a generic type at all here, just to be sure
	@Override
	...
	public CacheEntry getCacheEntry(String key) throws IOException {
		CacheEntry entry = getCacheEntry(key, null);
		if (entry == null)
			throw new CacheException("there is no valid cache entry with key [" + key + "]");
		return entry;
	}
	 ...
	 * remove all entries
	 * 
	 * @return returns the count of the removal or -1 if this information is not
	 *         available
	 */
	@Override
